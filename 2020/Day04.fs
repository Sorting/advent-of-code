...
module Day04 =
    ...
    let isWithinRange (value: string) min max = 
    ...
    let hasValidFieldValue key value = 
        ...
        | "byr", value -> isWithinRange value 1920 2002
        | "iyr", value -> isWithinRange value 2010 2020
        | "eyr", value -> isWithinRange value 2020 2030
        | "hgt", value ->
            match value with
            | Regex "^(\\d+)(cm|in)$" [height; unit] ->
                    match unit with
                    | "cm" -> isWithinRange height 150 193
                    | "in" -> isWithinRange height 59 76
                    | _ -> false
            | _ -> false
