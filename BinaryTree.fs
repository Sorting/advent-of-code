...
let findCommonRoot a b root =        
    ...
    let aPath = match findPath a root with Some (path, _) -> path | None -> []
    let bPath = match findPath b root with Some (path, _) -> path | None -> []

    findNode (match (aux Unchecked.defaultof<'a> (aPath, bPath)) with Some x -> x  | None -> Unchecked.defaultof<'a>) root

let countSteps a b = function
    | Empty -> 0
    | root ->
        let aSteps = match findPath a root with Some (_, steps) -> steps | None -> 1
        let bSteps = match findPath b root with Some (_, steps) -> steps | None -> 1
        (aSteps - 1) + (bSteps - 1)
