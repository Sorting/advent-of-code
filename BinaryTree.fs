...
let rec addNode parentValue value = function
    ...
    | Node(x, left, Empty) ->
        ...
        else Node (x, addNode parentValue value left, Empty)
    | Node(x, Empty, right) ->
        if x = parentValue 
        then Node(x, Node(value, Empty, Empty), right)
        else Node (x, Empty, addNode parentValue value right)
    | Node(x, left, right) ->
        if x = parentValue
        then failwith "Already occupied"
        else Node(x, addNode parentValue value left, addNode parentValue value right)
