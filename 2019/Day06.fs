...
module Day06 =    
    ...
    let findRoot = function
        ...
        | list ->
            ...
            |> Seq.head
            |> snd
            |> fun (a, b) -> BinaryTree.Node(a, BinaryTree.Node(b, BinaryTree.Empty, BinaryTree.Empty), BinaryTree.Empty)       
    ...
    let buildTree pairs =
        let rec aux tree = function
            ...
            | (a, b)::xs -> 
                if not (BinaryTree.exists a tree)                
                then aux tree (xs @ [(a, b)])
                else aux (tree |> BinaryTree.add a b) xs
