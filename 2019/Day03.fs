...
module Day03 =
    ...
    type Position =
        | IntersectionPoint of int * int
        | Point of WireType * int * int
    ...
    let rec buildGrid map currentPosition steps wireType =
        ...
        | direction :: xs ->
            ...
            let map' =
                List.fold (fun acc (x, y, currentSteps) ->
                    ...
                    | Some (Point (wt, firstWireSteps, secondWireSteps)) when wt <> wireType ->
                        let steps' =
                            (if wireType = FirstWire then
                                (currentSteps, secondWireSteps)
                             else
                                 (firstWireSteps, currentSteps))

                        acc
                        |> Map.remove (x, y)
                        |> Map.add (x, y) (IntersectionPoint steps')
                    | None ->
                        let firstWireSteps, secondWireSteps =
                            if wireType = FirstWire then (currentSteps, 0) else (0, currentSteps)

                        acc
                        |> Map.add (x, y) (Point(wireType, firstWireSteps, secondWireSteps))
                    | _ -> acc) map
                    (newPositions
                     |> List.take (newPositions.Length - 1))
            ...
            | Some (Point (wt, firstWireSteps, secondWireSteps)) ->
                let steps' =
                    if wireType = FirstWire then (currentSteps, secondWireSteps) else (firstWireSteps, currentSteps)

                buildGrid
                    (map'
                     |> Map.remove nextPosition
                     |> Map.add nextPosition (IntersectionPoint steps')) nextPosition currentSteps wireType xs
            | _ ->
                let x, y =
                    if wireType = FirstWire then (currentSteps, 0) else (0, currentSteps)
                ...
                buildGrid
                    (map'
                     |> Map.add nextPosition (Point(wireType, x, y))) nextPosition currentSteps wireType xs
